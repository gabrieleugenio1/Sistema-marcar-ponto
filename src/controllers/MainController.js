...
const { Usuario, Funcionarios, Codigo } = require("../models/Models");
...
module.exports = class PrincipalController {
    ...
    static async enviarSenha(req, res) {
        ...
        if(usuarioEmail){
            const codigoEmail = await Codigo.create({codigo:codigo, dataGerada:fn('NOW'), horarioGerado:fn('NOW'), ativo:true,usuarioId:usuarioEmail.id}); 
            ...
            const codigoEmail = await Codigo.create({codigo:codigo, dataGerada:fn('NOW'), horarioGerado:fn('NOW'), ativo:true,funcionarioMatricula:funcionarioEmail.matricula}); 
            const link = req.headers.host + '/esqueci-senha?codigo=' + codigoEmail.codigo;
